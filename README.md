...
- The runtime path is deterministic: no external LLM calls, no random calls, no model-generated verdicts.
- Evaluation is explicit: 103 labeled items across 11 fixtures, with issue-specific recall and false positives tracked separately.
...
Clauses resolve onto an explicit domain model, not free text. The objects are a Clause (carrying its exact character offsets in the source), a Finding, an escalation Rule, and a Party (ten roles, including the unknown and both-parties cases real contracts force on you). They link the way you would expect. A Finding points at the Clause it came from. A Clause points at its parent clause for document structure. An original clause links to its redline counterpart for version diffs. A Finding traces back through the detector that fired to the Rule that governs it. Each Finding also carries the party it disadvantages (discloser, recipient, both, or unclear), resolved deterministically rather than inferred by a model.

Under all of it sits the part that keeps the system honest: a governed risk taxonomy of 82 entries across 15 categories, guarded against duplicate IDs, that every detector has to classify into. Training-data use without opt-in, source-code access, residual knowledge, broad affiliate or subcontractor disclosure, perpetual confidentiality, liquidated-damages multipliers, unlimited or consequential liability, counterparty-friendly jurisdiction, protections lost on downstream agreements, definition-scope overbreadth, standard carve-outs missing, downstream-disclosure adequacy, compelled-disclosure protections, license-grant denials, warranty disclaimers, assignment asymmetry, and regulatory review areas (export control, Korean subcontract technical-data, downstream-contract conflict). The taxonomy is the closed vocabulary. Detectors classify into it, rules reason over it, reports render from it, and nothing reaches a reader that isn't tied to a taxonomy entry plus the clause span that triggered it. Taxonomy-to-registry drift is structurally gated to zero; an ID cannot live in one without the other.
